...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String caminhoInicial = "LAPR1FinalProject/";
        String input = "";
        float escolha = 0;
        String listaFicheiros[] = new String[10];

        if (args.length == 0) {
            File fileSIR = new File(caminhoInicial);
            if (fileSIR.exists() && fileSIR.isDirectory()) {
                File[] files = fileSIR.listFiles();
                for (File file : files) {
                    if (file.getName().endsWith(".csv") && dias < 10) {
                        listaFicheiros[dias] = file.getName();
                        dias++;
                    }
                }
            }
            ...
            System.out.println("\u001B[1mQue ficheiro deseja fazer?\u001B[0m");
            dias = 0;
            for(int i = 0; i < 10; i++) {
                if(listaFicheiros[i] != null){
                    System.out.println((i+1) + listaFicheiros[i]);
                    dias ++;
                }
            }
            escolha = scanner.nextFloat()-1;
            while(escolha < 0 || escolha >= dias || escolha != (int) escolha){
                mensagemErro(4);
                escolha = scanner.nextFloat()-1;
            }

            caminhoInicial += listaFicheiros[(int) escolha];
            dias = 0;
            escolha = 0;
